...
/* Markup that is wrong in ways the page does not show.
   ...
   It starts with one check, and is named for the family rather than for
   that check, because unlabelled form fields and header-less tables report
   into this same panel when they are built. Renaming a checker after it
   ships costs a store listing, another review, and a name its users have to
   unlearn.
...
(() => {
  ...
  if (duplicated.length > 0) {
    ...
    for (const [id, count] of duplicated) {
      ...
      /* textContent, not insertAdjacentHTML: the id comes from the page and
